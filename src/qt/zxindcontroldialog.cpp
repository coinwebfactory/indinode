...
#include "ui_zxindcontroldialog.h"
...
std::list<std::string> ZXindControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZXindControlDialog::listMints;

ZXindControlDialog::ZXindControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZXindControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZXindControlDialog::~ZXindControlDialog()
{
    delete ui;
}

void ZXindControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZXindControlDialog::updateList()
{
...
void ZXindControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZXindControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZXind_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZXindControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZXindControlDialog::GetSelectedMints()
{
...
void ZXindControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
