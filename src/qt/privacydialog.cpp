                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zXIND ought to be enough for anybody." - Bill Gates, 2017
    ui->zXINDpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzXINDSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzXIND->setEnabled(false);
        ui->pushButtonMintzXIND->setToolTip(tr("zXIND is currently disabled due to maintenance."));

        ui->pushButtonSpendzXIND->setEnabled(false);
        ui->pushButtonSpendzXIND->setToolTip(tr("zXIND is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zXINDpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzXIND_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXIND is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zXIND...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zXIND in ") +
...
void PrivacyDialog::on_pushButtonSpendzXIND_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXIND is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zXIND
        sendzXIND();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zXIND
    sendzXIND();
}

void PrivacyDialog::on_pushButtonZXindControl_clicked()
{
    ZXindControlDialog* zXindControl = new ZXindControlDialog(this);
    zXindControl->setModel(walletModel);
    zXindControl->exec();
}

void PrivacyDialog::setZXindControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzXindSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzXIND()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid IndiNode Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zXINDpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zXIND is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " XIND </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zXIND</b>";
    ...
    // use mints from zXind selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZXindControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZXindControlDialog::GetSelectedMints();
    }
    ...
    // Clear zxind selector in case it was used
    ZXindControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zXind Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Xind, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zXind Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zXIND </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zXIND "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zXIND "));
    ui->labelzXINDAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzXINDSyncStatus->setVisible(fShow);
}
