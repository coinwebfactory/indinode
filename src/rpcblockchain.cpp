...
Object blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zxindObj.push_back(Pair(to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zxindObj.emplace_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.emplace_back(Pair("zXINDsupply", zxindObj));
...
Value getrawmempool(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in indinode\n"
...
Value getblock(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "  \"merkleroot\" : \"xindx\", (string) The merkle root\n"
            ...
            "  \"difficulty\" : x.xind,  (numeric) The difficulty\n"
...
Value gettxoutsetinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"total_amount\": x.xind          (numeric) The total amount\n"
            "}\n"
            "\nExamples:\n" +
...
Value gettxout(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "  \"value\" : x.xind,           (numeric) The transaction value in btc\n"
            "  \"scriptPubKey\" : {         (json object)\n"
            "     \"asm\" : \"code\",       (string) \n"
            "     \"hex\" : \"hex\",        (string) \n"
            "     \"reqSigs\" : n,          (numeric) Number of required signatures\n"
            "     \"type\" : \"pubkeyhash\", (string) The type, eg pubkeyhash\n"
            "     \"addresses\" : [          (array of string) array of indinode addresses\n"
            "     \"indinodeaddress\"   	 	(string) indinode address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
...
Value getblockchaininfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"chain\": \"xindx\",        (string) current network name as defined in BIP70 (main, test, regtest)\n"
            "  \"blocks\": xindxind,         (numeric) the current number of blocks processed in the server\n"
            "  \"headers\": xindxind,        (numeric) the current number of headers we have validated\n"
            "  \"bestblockhash\": \"...\", (string) the hash of the currently best block\n"
            "  \"difficulty\": xindxind,     (numeric) the current difficulty\n"
            "  \"verificationprogress\": xindx, (numeric) estimate of verification progress [0..1]\n"
            "  \"chainwork\": \"xindx\"     (string) total amount of work in active chain, in hexadecimal\n"
            "}\n"
            "\nExamples:\n" +
...
Value getchaintips(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"height\": xindx,         (numeric) height of the chain tip\n"
            "    \"hash\": \"xindx\",         (string) block hash of the tip\n"
            ...
            "    \"status\": \"xindx\"        (string) status of the chain (active, valid-fork, valid-headers, headers-only, invalid)\n"
            "  }\n"
            "]\n"
...
Value getmempoolinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"size\": xindxx                (numeric) Current tx count\n"
            "  \"bytes\": xindxx               (numeric) Sum of all tx sizes\n"
            "}\n"
            "\nExamples:\n" +
