...
// Copyright (c) 2017-2018 The IndiNode developers
...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"version\": xindxx,           (numeric) the server version\n"
            "  \"protocolversion\": xindxx,   (numeric) the protocol version\n"
            "  \"walletversion\": xindxx,     (numeric) the wallet version\n"
            "  \"balance\": xindxindx,         (numeric) the total indinode balance of the wallet (excluding zerocoins)\n"
            "  \"zerocoinbalance\": xindxindx, (numeric) the total zerocoin balance of the wallet\n"
            "  \"blocks\": xindxind,           (numeric) the current number of blocks processed in the server\n"
            "  \"timeoffset\": xindxx,        (numeric) the time offset\n"
            "  \"connections\": xindxx,       (numeric) the number of connections\n"
            "  \"proxy\": \"host:port\",     (string, optional) the proxy used by the server\n"
            "  \"difficulty\": xindxind,       (numeric) the current difficulty\n"
            "  \"testnet\": true|false,      (boolean) if the server is using testnet or not\n"
            "  \"moneysupply\" : \"supply\"       (numeric) The money supply when this block was added to the blockchain\n"
            "  \"zXINDsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zXIND denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zXIND denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zXIND denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zXIND denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zXIND denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zXIND denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zXIND denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zXIND denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zXIND denominations\n"
            "  }\n"
            ...
            "  \"paytxfee\": x.xindx,         (numeric) the transaction fee set in indinode/kb\n"
            "  \"relayfee\": x.xindx,         (numeric) minimum relay fee for non-free transactions in indinode/kb\n"
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zxindObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zxindObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zXINDsupply", zxindObj));
...
Value mnsync(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1 || (strMode != "status" && strMode != "reset")) {
        throw runtime_error(
            ...
            "  \"lastMasternodeList\": xindx,        (numeric) Timestamp of last MN list message\n"
            "  \"lastMasternodeWinner\": xindx,      (numeric) Timestamp of last MN winner message\n"
            "  \"lastBudgetItem\": xindx,            (numeric) Timestamp of last MN budget message\n"
            "  \"lastFailure\": xindx,           (numeric) Timestamp of last failed sync\n"
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"indinodeaddress\"\n"
            "\nReturn information about the given indinode address.\n"
            "\nArguments:\n"
            "1. \"indinodeaddress\"     (string, required) The indinode address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"indinodeaddress\", (string) The indinode address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: IndiNode address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are indinode addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) indinode address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"indinodeaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"indinodeaddress\"  (string, required) The indinode address to use for the signature.\n"
